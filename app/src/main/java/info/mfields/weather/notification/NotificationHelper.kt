...
class NotificationHelper(private val c: Context) {
    ...
    fun updateFromCache() = runBlocking {
        ...
        if (!s.notificationsEnabled) return@runBlocking
        val ch = NotificationChannel("forecast","Forecast",NotificationManager.IMPORTANCE_LOW); nm.createNotificationChannel(ch)
