...
class WeatherGovClient {
    ...
    fun points(lat: Double, lon: Double): PointProperties {
        ...
        val req = Request.Builder().url(url)
            .header("User-Agent", "simple-weather/1.0 (mfields info@mfields.info)")
            .build()
        c.newCall(req).execute().use { r ->
            check(r.isSuccessful) { "Points request failed: ${r.code}" }
            return j.decodeFromString<PointsResponse>(r.body!!.string()).properties.copy(pointsUrl = url)
        }
    }
...
@Serializable data class PointProperties(
