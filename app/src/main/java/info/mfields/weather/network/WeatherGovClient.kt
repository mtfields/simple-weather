...
class WeatherGovClient {
    ...
    fun points(lat: Double, lon: Double): PointProperties {
        ...
        c.newCall(req).execute().use { r ->
            if (!r.isSuccessful) error("Points request failed: ${r.code}")
    ...
    fun hourly(url: String): List<Period> {
        val req = Request.Builder().url(url)
            .header("User-Agent", "simple-weather/1.0 (mfields info@mfields.info)").build()
        c.newCall(req).execute().use { r ->
            if (!r.isSuccessful) error("Hourly request failed: ${r.code}")
    ...
    fun summarize(periods: List<Period>): List<DaySummary> = periods.groupBy { OffsetDateTime.parse(it.startTime).toLocalDate() }
        .entries.sortedBy { it.key }.take(5).map { (d, ps) ->
            DaySummary(
                ...
                ps.maxOf { it.temperature },
                ps.minOf { it.temperature },
                ps.groupingBy { it.shortForecast }.eachCount().maxByOrNull { it.value }?.key ?: ""
            )
        }
}
