...
class WeatherGovClient {
    ...
    fun points(lat: Double, lon: Double): PointProperties {
        val req = Request.Builder().url("https://api.weather.gov/points/$lat,$lon")
            .header("User-Agent", "simple-weather/1.0 (mfields info@mfields.info)").build()
        c.newCall(req).execute().use { r ->
            check(r.isSuccessful) { "points request failed: ${r.code}" }
            return j.decodeFromString<PointsResponse>(r.body!!.string()).properties
        }
    }
    ...
    fun hourly(url: String): List<Period> {
        ...
        c.newCall(req).execute().use { r ->
            check(r.isSuccessful) { "hourly request failed: ${r.code}" }
            return j.decodeFromString<HourlyResponse>(r.body!!.string()).properties.periods
        }
    }
}
@Serializable data class PointsResponse(val properties: PointProperties)
@Serializable data class PointProperties(val forecastHourly: String, val gridId: String? = null, val gridX: Int? = null, val gridY: Int? = null, val @Suppress("PropertyName") cwa: String? = null)
