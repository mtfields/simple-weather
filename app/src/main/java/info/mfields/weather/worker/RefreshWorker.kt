...
class RefreshWorker(ctx: Context, params: WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    companion object {
        ...
        suspend fun scheduleFromState(context: Context) {
            val s = Store(context).state.first()
            if (s.backgroundEnabled) schedule(context, s.refreshIntervalHours.toLong()) else cancel(context)
        }
    }
}
